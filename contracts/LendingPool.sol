...
contract LendingPool {
    ...
    event Lend(address indexed _borrower, address indexed _lender, uint _amount);
    event LendingAccepted(address indexed _lender, address indexed _borrower, uint _amount);
    ...
    modifier withCorrectLendingParams(address account, uint amount) {
        ...
        require(msg.sender != account, "You cannot lend or close lending to yourself");
        _;
    }

    function lendFrom(address lender, uint amount) withCorrectLendingParams(lender, amount) external {
    ...
    function acceptLendingFrom(address borrower, uint amount) withCorrectLendingParams(borrower, amount) external {
    ...
    function totalLendingAmountOf(address account) external view returns (uint) {
